"""
Demand Forecasting Module for Light Vehicle Market
...
class DemandForecaster:
    """Forecasts demand for light vehicle market, EV, and ICE segments"""
    ...
    def forecast_market(
        ...
        """
        Forecast total light vehicle market demand using linear extrapolation with CAGR cap
    ...
    def forecast_ev(
        ...
        """
        Forecast EV light vehicle demand using logistic adoption model
    ...
    def forecast_ice(
        ...
        """
        Forecast ICE light vehicle demand as residual
...
def run_demand_forecast(
    ...
    # Get product names from data_loader's vehicle_config
    products = data_loader.vehicle_config.get_product_names()
    lifetime_years = data_loader.vehicle_config.get_lifetime_years()

    # Load historical data
    market_years, market_hist = data_loader.get_demand_data(products['market'], region)
    ev_years, ev_hist = data_loader.get_demand_data(products['ev'], region)
    ...
    if track_fleet:
        try:
            # Calculate fleet evolution (vehicle-specific lifetime)
            ev_fleet = forecaster.calculate_fleet_evolution(
                ev_demand, years, lifetime_years=lifetime_years
            )
            ice_fleet = forecaster.calculate_fleet_evolution(
                ice_demand, years, lifetime_years=lifetime_years
            )
...
if __name__ == "__main__":
    ...
    print("Testing Light Vehicle Demand Forecasting...")
    print("This module is designed to be imported and used with vehicle-specific configurations.")
    print("\nExample usage:")
    print("  from vehicle_config import VehicleConfig")
    print("  from common.data_loader import DataLoader")
    print("  from common.cost_analysis import run_cost_analysis")
    print("  from common.demand_forecast import run_demand_forecast")
    print("  ")
    print("  config = VehicleConfig('two_wheeler')")
    print("  loader = DataLoader(config)")
    print("  cost_result = run_cost_analysis(loader, 'China', end_year=2030)")
    print("  demand_result = run_demand_forecast(loader, 'China', cost_result['tipping_point'])")
