"""
Utility Functions for Commercial Vehicle Demand Forecasting
Provides smoothing, CAGR calculation, forecasting, and validation utilities
Supports three powertrains: EV, ICE, and NGV
"""
...
def validate_forecast_consistency_three_powertrain(
    market: np.ndarray,
    ev: np.ndarray,
    ice: np.ndarray,
    ngv: np.ndarray,
    ...
    """
    Validate forecast consistency for three powertrains (EV, ICE, NGV)

    Args:
        market: Market demand array
        ev: EV demand array
        ice: ICE demand array
        ngv: NGV demand array
        tolerance: Tolerance for sum check (1% default)
    ...
    """
    # Check non-negativity
    if np.any(market < 0) or np.any(ev < 0) or np.any(ice < 0) or np.any(ngv < 0):
        return False, "Negative values detected in forecast"

    # Check sum consistency: EV + ICE + NGV should equal Market (within tolerance)
    total = ev + ice + ngv
    relative_diff = np.abs(total - market) / np.maximum(market, 1)

    if np.any(relative_diff > tolerance):
        max_diff_idx = np.argmax(relative_diff)
        return False, f"Sum consistency violated at index {max_diff_idx}: EV+ICE+NGV={total[max_diff_idx]:.0f}, Market={market[max_diff_idx]:.0f}"

    # Check that individual powertrains don't exceed market
    if np.any(ev > market) or np.any(ice > market) or np.any(ngv > market):
        return False, "EV, ICE, or NGV demand exceeds market demand"

    # Check that shares sum to approximately 1.0
    shares = (ev + ice + ngv) / np.maximum(market, 1)
    if np.any(shares > 1.0 + tolerance):
        return False, "Total powertrain shares exceed 100%"
...
def exponential_decline(
    ...
    """
    Calculate exponential decline from a starting point

    Used for NGV chimera modeling: share declines with exponential decay

    Args:
        t: Time array (years)
        t0: Start time for decline
        initial_value: Value at t0
        half_life: Half-life in years

    Returns:
        Array of values following exponential decline
    """
    ...
    for i, time in enumerate(t):
        if time <= t0:
            ...
            years_since = time - t0
            decay_factor = np.exp(-np.log(2) * years_since / half_life)
            result[i] = initial_value * decay_factor
