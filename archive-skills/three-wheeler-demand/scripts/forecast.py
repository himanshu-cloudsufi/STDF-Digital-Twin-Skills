"""
Main Forecasting Module for Three-Wheeler Demand
...
class ForecastOrchestrator:
    """Main class to orchestrate the three-wheeler forecasting process"""
    ...
    def forecast_region(self, region: str) -> Dict[str, any]:
        ...
        """
        print(f"\n{'='*70}")
        print(f"Three-Wheeler Demand Forecast for Region: {region}")
        print(f"{'='*70}")
    ...
    def forecast_global(self) -> Dict[str, any]:
        ...
        """
        print("\n" + "="*70)
        print("GLOBAL THREE-WHEELER DEMAND FORECASTING")
        print("="*70)
...
def main():
    """Main entry point for three-wheeler demand forecasting"""
    import argparse

    parser = argparse.ArgumentParser(
        description='Three-Wheeler Demand Forecasting with EV Disruption Analysis'
    )
    parser.add_argument(
        '--region',
        type=str,
    ...
    # Export results
    base_filename = f"three_wheeler_{args.region}_{args.end_year}"
    ...
    if args.output in ['csv', 'both']:
        if args.region == "Global":
            ...
            for region_name, region_result in result['regional_results'].items():
                csv_path = os.path.join(args.output_dir, f"three_wheeler_{region_name}_{args.end_year}.csv")
    ...
    print("\n" + "="*70)
    print("Three-Wheeler Demand Forecasting Complete!")
    print("="*70)
